...
def move(event):
    ...
    #Change the coords of the CURRENT object based on the event.y position of the
    mouse. Recall the ball has size 10
...
for i in range(1,5):
    heights['height_%d' % i].config(text="Height: " + str(round(415 -
    main_canvas.coords(balls['ball_%d' % i])[3], 2)))
...
def step(t):
    ...
    for i in range(1,5):
    #DO THE PHYSICS! Negate a and v because canvas y values increase as you move
    down.
    ...
    data['data_%d' % i].append((time, 415 - main_canvas.coords(balls['ball_%d' %
    i])[3]))
...
#Balls may start on the ground or at the top of the screen so call step() at least
once
step(t_slider.get())

#Run step() until ALL balls have hit the ground or left the screen based of the y2
coord [3]
while 15 < main_canvas.coords(balls['ball_1'])[3] < 415 or 15 <
main_canvas.coords(balls['ball_2'])[3] < 415 or 15 <
main_canvas.coords(balls['ball_3'])[3] < 415 or 15 <
main_canvas.coords(balls['ball_4'])[3] < 415:
step(t_slider.get())
...
def reset():
"""Erase all "DASH" tags from canvas, set balls back to ground, and resent entry
fields."""
global time
...
#Time slider
t_slider = tkinter.Scale(input_frame, from_=0, to=1, tickinterval=.1, resolution=.01,
orient=HORIZONTAL)
...
command=lambda:step(t_slider.get()))
